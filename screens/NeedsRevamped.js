...
const NeedsRevamped = ({ navigation }) => {
  ...
  const loadKnowledge = useCallback(async () => {
    try {
      ...
      const needsList = await DataService.getFeelingAndNeedsQuestions("needs-questions");
      dispatch({ type: actionTypes.SET_KNOWLEDGE, payload: needsList });
    } catch (error) {
  ...
  const handleAddMainQuestion = async () => {
    ...
    try {
      ...
      await DataService.addDocument("needs-questions", newQuestion, newQuestionId);
      dispatch({ type: actionTypes.SET_MAIN_QUESTION, payload: "" });
      loadKnowledge();
      Alert.alert("Success", "Main question created with 9 subquestions");
    } catch (error) {
  ...
  const handleShowSubItems = async (subquestion) => {
    ...
    // Load answer options
    try {
      const options = await DataService.getAnswerOptions(subquestion.id, 'needs-questions');
  ...
  const handleAddAnswerOption = async () => {
    ...
    try {
      ...
      // Refresh options
      const options = await DataService.getAnswerOptions(state.selectedSubquestionId, 'needs-questions');
      dispatch({ type: actionTypes.SET_ANSWER_OPTIONS, payload: options || [] });
  ...
  const handleDeleteAnswerOption = async (optionId) => {
    try {
      await DataService.deleteAnswerOption(optionId, state.selectedSubquestionId, 'needs-questions');
