...
const FeelingsRevamped = ({ navigation }) => {
  ...
  const loadContainers = useCallback(async () => {
    try {
      ...
      const containerList = await DataService.getBoxContainers("feelings-boxes");
      ...
      if (auth.currentUser) {
        const selection = await DataService.getUserSelection("user-feelings-selections", auth.currentUser.uid);
  ...
  const handleAddContainer = async () => {
    ...
    try {
      ...
      const newContainer = {
        title: state.containerTitle || "Feelings",
        containerId: newContainerId,
        boxes,
      };

      console.log("Creating new container:", newContainer);
      await DataService.addDocument("feelings-boxes", newContainer, newContainerId);
  ...
  const handleUpdateBox = async () => {
    ...
    try {
      ...
      dispatch({ type: actionTypes.SET_UPDATE_BOX, payload: initialState.updateBox });
      loadContainers();
    } catch (error) {
  ...
  const handleShowSubBoxes = async (box) => {
    dispatch({ type: actionTypes.SET_SELECTED_BOX, payload: box.id });
  ...
  const handleSubBoxSelect = async (subBox) => {
    if (state.isAdmin) {
      ...
      try {
        ...
        await DataService.saveUserSelection("user-feelings-selections", selection);
        dispatch({ type: actionTypes.SET_USER_SELECTION, payload: selection });
    ...
    return (
      <>
        <View style={styles.containerHeader}>
          <Text style={styles.containerTitle}>{item.title || "Feelings"}</Text>
          {state.userSelection && (
          ...
          <View style={styles.bottomContainer}>
            <View style={styles.inputContainer}>
              <TextInput
                ...
                onChangeText={(text) => 
                  dispatch({ type: actionTypes.SET_CONTAINER_TITLE, payload: text })
                }
              />
              <TouchableOpacity onPress={handleAddContainer} style={styles.sendButton}>
